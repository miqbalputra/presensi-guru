...
let warmupPromise: Promise<UserLocation> | null = null

const normalizePosition = (position: GeolocationPosition): UserLocation => ({
...
export const getLocationErrorMessage = (error: { code?: number; message?: string } | null | undefined) => {
  if (!error) {
...
export const getUserLocation = (options: LocationOptions = {}): Promise<UserLocation> => {
  const {
    timeout = 12000,
    maximumAge = 10000,
...
export const getReliableUserLocation = async (options: LocationOptions = {}): Promise<UserLocation | null> => {
  const {
    minAccuracy = 80,
    cacheMaxAgeMs = 15000,
    firstTimeout = 12000,
