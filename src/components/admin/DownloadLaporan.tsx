...
function DownloadLaporan() {
  ...
  const downloadExcel = () => {
    ...
    const exportData: Record<string, any>[] = logs.map(log => ({
      'Tanggal': log.tanggal,
    ...
    addJsonSheet(wb, 'Laporan Presensi', exportData)
    downloadWorkbook(wb, fileName).then(() => showNotification('Laporan Excel berhasil didownload!')).catch((error) => showNotification('Gagal export Excel: ' + error.message))
  }
  ...
  const downloadSemuaGuruExcel = () => {
    ...
    addJsonSheet(wb, 'Ringkasan Kehadiran', summaryData)
    ...
    if (latePiketData.length > 0) {
      addJsonSheet(wb, 'Terlambat Piket', latePiketData)
    }
    ...
    if (earlyData.length > 0) {
      addJsonSheet(wb, 'Pulang Awal Piket', earlyData)
    }
    ...
    if (izinSakitData.length > 0) {
      addJsonSheet(wb, 'Alasan Izin Sakit', izinSakitData)
    }
    ...
    if (forgottenData.length > 0) {
      addJsonSheet(wb, 'Lupa Checkout', forgottenData)
    }
    ...
    dataGuru.forEach(guru => {
      ...
      addJsonSheet(wb, sheetName, detailData)
    })

    const fileName = `Laporan_Semua_Guru_${formatDate(new Date())}.xlsx`
    downloadWorkbook(wb, fileName).then(() => showNotification('Laporan semua guru berhasil didownload!')).catch((error) => showNotification('Gagal export Excel: ' + error.message))
  }
        ...
        <div className="bg-white rounded-lg shadow p-6 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4 pt-4 border-t">
            ...
            <div className="bg-blue-50 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-blue-600">{startDate && endDate ? Math.ceil((new Date(endDate).getTime() - new Date(startDate).getTime()) / (1000 * 60 * 60 * 24)) + 1 : 0}</p>
                        ...
                        <tr key={guru.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-center">
                            <span className={`inline-flex items-center justify-center px-3 py-1 rounded-full text-sm font-semibold ${
                              Number(persentase) >= 80 ? 'bg-green-100 text-green-800' :
                              Number(persentase) >= 60 ? 'bg-yellow-100 text-yellow-800' :
